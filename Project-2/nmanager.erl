...
-export([fullNetwork/2,line/2,grid2d/2,gspawner_Nodes/2,psspawner_Nodes/3,listener/3,imperfect3d/2,divider/2]).
...
lists:foreach(fun(Elem)->
    ...
    Mod = Ind rem Rows,
    Random = lists:nth(rand:uniform(length(Otherlists)), Otherlists),
    if
        Ind =< Rows -> %For all the bottom elements
            if
                Mod ==1  -> %left most value
                    Elem ! {populate,[lists:nth(Ind+1,PIDList),lists:nth(Ind+Rows,PIDList),lists:nth(Ind+Rows+1,PIDList),Random]};
                Mod == 0-> %right most 
                    Elem ! {populate,[lists:nth(Ind-1,PIDList),lists:nth(Ind+Rows,PIDList),lists:nth(Ind+Rows-1,PIDList),Random]};
                true -> %middle values
                    Elem ! {populate,[lists:nth(Ind+1,PIDList),lists:nth(Ind-1,PIDList),lists:nth(Ind+Rows,PIDList),lists:nth(Ind+Rows-1,PIDList),lists:nth(Ind+Rows+1,PIDList),Random]}
            end;
        ...
        true -> %For all the middle elements
            if
                Mod ==1  ->
                    ...
                    lists:nth(Ind-Rows+1,PIDList),Random]};
                Mod == 0->
                    Elem ! {populate,[lists:nth(Ind-1,PIDList),lists:nth(Ind+Rows,PIDList),lists:nth(Ind-Rows,PIDList),lists:nth(Ind-Rows-1,PIDList),lists:nth(Ind-Rows-1,PIDList),Random]};
                true ->
                    Elem ! {populate,[lists:nth(Ind+1,PIDList),lists:nth(Ind-1,PIDList),lists:nth(Ind+Rows,PIDList),lists:nth(Ind-Rows+1,PIDList),lists:nth(Ind-Rows-1,PIDList),lists:nth(Ind+Rows+1,PIDList),lists:nth(Ind+Rows-1,PIDList),lists:nth(Ind-Rows,PIDList),Random]}
            end
    end
    end,PIDList).
